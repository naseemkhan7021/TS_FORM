<div class="modal fade addSubcause" wire:ignore.self tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <form wire:submit.prevent="save">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Sub-cause Description"
                            wire:model="sub_causes_description">
                        <span class="text-danger"> @error('sub_causes_description')
                                {{ $message }}@enderror</span>
                    </div>
                    <div class="form-group">
                        <label for="">Sub-cause Abbrivation</label>
                        <input type="text" class="form-control" placeholder="Sub-cause Abbrivation"
                            wire:model="sub_causes_abbr">
                        <span class="text-danger"> @error('sub_causes_abbr') {{ $message }}@enderror</span>
                    </div>
                    <div class="form-group">
                        <label for="" class="form-label">Cause Description</label>
                        <select wire:model="sub_causes_fk" class="form-control">
                            <option value="0" hidden>Select main Cause</option>
                            @forelse ($cousesData as $item)
                                <option value="{{ $item->causes_id }}">{{ $item->causes_description }}</option>
                            @empty
