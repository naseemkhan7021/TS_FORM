<div>
    ...
    <table class="table display table-bordered data-table text-center" style="width:100%">
        ...
        <tbody>
                ...
                <tr style="overflow: hidden;">
                    ...
                    <td>
                        ...
                        @if ($row->sbc_logo_small && Storage::disk()->exists($row->sbc_logo_small))
                            <a data-toggle="modal" data-target="#imgview"
                                wire:click="$set('showimg','{{ $row->sbc_logo_small }}')" style="height: 2rem"
                                href="javascript:void(0)" class="d-block text-center"> <img class="h-100"
                                    src="{{ Storage::url($row->sbc_logo_small) }}"
                                    alt="{{ Storage::url($row->sbc_logo_small) }}"></a>
                        @else
                            No Image Selected
                        @endif
                    </td>
                    <td>
                        @if ($row->sbc_logo_large && Storage::disk()->exists($row->sbc_logo_large))
                            <a data-toggle="modal" data-target="#imgview"
                                wire:click="$set('showimg','{{ $row->sbc_logo_large }}')" style="height: 2rem"
                                href="javascript:void(0)" class="d-block text-center"> <img class="h-100"
                                    src="{{ Storage::url($row->sbc_logo_large) }}"
                                    alt="{{ Storage::url($row->sbc_logo_large) }}"></a>
                        @else
                            No Image Selected
                        @endif
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="8">'{{ $data_not_found }}</td>
                </tr>
            @endforelse
